...
def extract_fact_and_vibe(image_paths: List[str], caption: str, hashtags: list):
    ...
    - 주관적 감성 (Vibe): 단순 정보만 전달하는 정보성 게시물이라면, 억지로 지어내지 말고 `vibe_text`를 빈 문자열("")로 둬. 객관적이지 않은 정보를 포함한 경우엔 검색 최적화 키워드(예: '느좋', '차분한', '퇴폐적인')를 문장에 풍부하게 녹여내. 절대 억지로 키워드를 욱여넣지말고 유형과 맥락을 참고해서 판단해.
