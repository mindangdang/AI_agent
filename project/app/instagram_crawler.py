...
from app.instagram_stealth import (
    apply_runtime_stealth as _apply_runtime_stealth,
    build_chrome_stealth_args as _build_chrome_stealth_args,
)
...
def crawl_instagram_post(
    ...
    """Crawl an Instagram post.

    Server-oriented behavior:
    - fast-fail on login/challenge walls
    - bounded waits to avoid long API latency
    - deterministic schema for callers
    """
    if not post_url:
...
def build_chrome_stealth_args(proxy: Optional[str] = None, user_agent: Optional[str] = None) -> List[str]:
    """Backward-compatible wrapper for stealth Chrome arguments."""
    return _build_chrome_stealth_args(proxy=proxy, user_agent=user_agent)


def apply_runtime_stealth(driver: WebDriver) -> None:
    """Backward-compatible wrapper for runtime stealth setup."""
    _apply_runtime_stealth(driver)
