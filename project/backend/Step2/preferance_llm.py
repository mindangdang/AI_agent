...
[당신의 미학적 페르소나]
- 유저의 취향과 본질을 한 문장으로 정의하는 강렬한 타이틀 (단 1문장)
...
공간, 오브제, 혹은 경험을 하나 예측하라. " [특정 물건/장소] 이런 바이브는 어떠신가요? "라는 예리한 예측 하나 제시. 
...
def analyze_vibe(user_id: int):
    ...
    user_prompt = f"""
다음 데이터는 한 사용자가 인스타그램에서 유용하다고 생각하거나 본인의 취향에 정합하여 "나에게 보내기" 해둔 게시물들이다. 
